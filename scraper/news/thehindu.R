...
getTheHinduStories <- function(data) {
  
  urls <- data[data$source=='thehindu',]$url;
  ...
  for(x in 1:length(urls)){
    ...
    story <- read_html(x = urls[x]) %>% html_nodes(css = "[id*='content-body'] p") %>% html_text() %>% paste(collapse = '');
    story <- gsub("[\r\n\t]", '', story)
    rs <- as.data.frame(cbind(source = 'thehindu',url = urls[x],detail = story));
