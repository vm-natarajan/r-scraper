...
getTimesOfIndiaStories <- function(data) {
  
  urls <- data[data$source=='timesofindia',]$url;
  ...
  for(x in 1:length(urls)){
    ...
    story <- read_html(x = urls[x]) %>% html_nodes(css = ".article_content div.Normal") %>% html_text() %>% paste(collapse = '');
    story <- gsub("[\r\n\t]", '', story)
    rs <- as.data.frame(cbind(source = 'timesofindia',url = urls[x],detail = story));
