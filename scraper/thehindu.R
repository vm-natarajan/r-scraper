getTheHinduSectionFeeds <- function(rdriver,url_section,section){
  ...
  page_source <- rdriver$getPageSource()[[1]]  %>% read_html()  %>% html_text(trim = TRUE) %>% read_xml() 
...
getTheHinduFeeds <- function(rdriver){
  ...
  for(x in c(1:length(sections))){
    section_set <- getTheHinduSectionFeeds(rdriver,url_sections[x],sections[x]); 
